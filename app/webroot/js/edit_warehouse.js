...
//            this.value = (ui.item ? ui.item.Product.name : '');
            ...
            if(ui.item.ProductOption.length > 0){
                pSku.val(ui.item.ProductOption[0].code);
            }else{
    ...
    function addProduct() {
        ...
        if (valid) {
            var qtyVal, optionNames = [], optionIds = [], price, subPrice,extendCode='';
            qtyVal = qty.val();
            ...
            $('.radio-op').each(function () {
                if ($(this).is(':checked')) {
                    ...
                    extendCode+=$(this).data('code');
                }
            });
            var duplicated = false;
            var code =  pSku.val()+extendCode;
            ...
            if (!duplicated) {
                var uuid = uniqId();
                var template = '<tr class="first-tr row' + uuid + '" data-id="' + pSku.val()+extendCode + '">' +
                    '<td>' + pSku.val()+extendCode + '</td>' +
                    '<td>' + pName.val() + '</td>' +
                    '<td><span class="price-text">' + digits(subPrice) + '</span></td>' +
                    '<td class="hidden-qty-text">' +
                    '<a href="javascript:;" class="price-down"><i class="icon icon-arrow-down"></i></a><input type="text" class="hidden-qty" data-price="' + subPrice + '" name="data[InoutWarehouseDetail][' + uuid + '][qty]" value="' + qtyVal + '"><a href="javascript:;"  class="price-up"><i class="icon icon-arrow-up"></i></a>' +
                    '</td>' +
