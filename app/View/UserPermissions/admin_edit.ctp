        ...
        <tr>
            ...
            <td align="right">
                ...
                if(in_array(strtolower($roleName), $adminRoleName)){//Admin
                    echo $this->Html->image('/img/icons/tick_disabled.png');
                }else{
                ?>
                <a href="javascript:;;" id="<?php echo 'permission-' . $acoPath[count($acoPath)-1]['Aco']['id'] . '-' . $data['id']; ?>" onclick="<?php if ($roleName != 'Administrator'): ?>acos.toggle('<?php echo $this->Html->url(array('admin'=>true,'controller'=>'user_permissions','action'=>'toggle'));?>',<?php echo $acoPath[count($acoPath)-1]['Aco']['id']; ?>, <?php echo $data['id']; ?>);return false;<?php endif; ?>">
                    <?php echo $this->Html->image('/img/icons/allow-' . $data['allowed'] . '.png'); ?>
                </a>
                <?php
                }
                ?>
